...
int main(int argc, char *argv[])
{
    ...
    glm::vec2 thisMousePos{0, 0};
    glm::vec2 bridgeVector{0, 0};
    float dist{0.0};

    auto t0 = std::chrono::high_resolution_clock::now();
    while (true)
    {
        auto& input = eventHander.fetchUserInput(true); //true blocks, false polls
        ...
        if (input.mButtonDown)
        {
            auto t1 = std::chrono::high_resolution_clock::now();
            auto elapsedTime = std::chrono::duration_cast<std::chrono::duration<float>>(t1 - t0).count();
            thisMousePos = {input.mouseX, input.mouseY};
            bridgeVector = thisMousePos - lastMousePos;
            dist = glm::length(bridgeVector);
            std::cout << "dist: " << dist << std::endl;
            if (dist > 2)
            {
                for (float step = 0.0; step < dist; step += 3.0)
                {
                    fragInput.mouseX = lastMousePos.x + bridgeVector.x * step / dist;
                    fragInput.mouseY = lastMousePos.y + bridgeVector.y * step / dist;
                    renderer.render(input.cmdId, true);
                }
            }
