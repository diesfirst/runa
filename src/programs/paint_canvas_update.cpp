...
int main(int argc, char *argv[])
{
    ...
    auto& swapchainPipe = renderer.createGraphicsPipeline(
            "swap", paintPLayout, "vert1", "swap", "swapchain", swapRenderArea, false);
    auto& palettePipe = renderer.createGraphicsPipeline(
            "palette", paintPLayout, "vert1", "uv00", "swapchain", swapRenderArea, false);
    auto& brushOnlyPipe = renderer.createGraphicsPipeline(
            "brush", paintPLayout, "vert1", "frag1", "offscreen", offscreenRenderArea, false);
    auto& voidPipe = renderer.createGraphicsPipeline(
            "void", paintPLayout, "vert1", "void", "offscreen", offscreenRenderArea, false);
    ...
    renderer.addRenderPassInstance("intermediate", "offscreen_clear", "brush");
    renderer.addRenderPassInstance("swap", "swapchain", "swap");
    renderer.addRenderPassInstance("swap", "swapchain", "palette");
    renderer.addRenderPassInstance("intermediate", "offscreen_clear", "void");
                        ...
                        case WindowEventType::Motion:
                            {
                                ...
                                glm::vec4 mPos = glm::vec4{
                                    input->getX() / float(WIDTH), input->getY() / float(HEIGHT), 1., 1.};
                                switch (state)
                                {
