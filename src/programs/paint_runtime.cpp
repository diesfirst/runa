...
void loadDefaultShaders(Renderer& renderer)
{
    //vert shaders
    const std::string shaderDir{SHADER_DIR};
    renderer.loadVertShader(shaderDir + "fullscreen_tri.spv", "vert1");
    renderer.loadFragShader(shaderDir + "spot_xform.spv", "frag1");
    renderer.loadFragShader(shaderDir + "composite_xform.spv", "swap");
    renderer.loadFragShader(shaderDir + "uv_00.spv", "uv00");
    renderer.loadFragShader(shaderDir + "void.spv", "void");
...
int main(int argc, char *argv[])
{
    ...
    auto& intermediate = renderer.createAttachment("intermediate", {C_WIDTH, C_HEIGHT},
            vk::ImageUsageFlagBits::eColorAttachment |
            vk::ImageUsageFlagBits::eSampled);
    ...
    swapShader.specData.integer1 = 1; 
    ...
    auto& brushOnlyPipe = renderer.createGraphicsPipeline(
            "brush", paintPLayout, quadShader, tarotShader, offScreenClearPass, offscreenRenderArea, false);
    auto& voidPipe = renderer.createGraphicsPipeline(
            "void", paintPLayout, quadShader, voidShader, offScreenClearPass, offscreenRenderArea, false);

    offscreenPipe.create();
    swapchainPipe.create();
    palettePipe.create();
    brushOnlyPipe.create();
    voidPipe.create();
    ...
    renderer.addFramebuffer(intermediate, offScreenClearPass, brushOnlyPipe);
    ...
    renderer.addFramebuffer(intermediate, offScreenClearPass, voidPipe);

    renderer.recordRenderCommands(0, {0, 1, 2});
    renderer.recordRenderCommands(1, {4, 2});
    renderer.recordRenderCommands(2, {3});
    renderer.recordRenderCommands(3, {1, 2});
